...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        try {
            ...
            if (this.contents[index].priority()
                    <= this.contents[parentIndex(index)].priority()) {
    ...
    private void sink(int index) {
        ...
        try {
            validateSinkSwimArg(index);
            if (this.contents[index].priority() >= this.contents[leftIndex(index)].priority()
                    || this.contents[index].priority() >= this.contents[rightIndex(index)].priority()) {
                int minI = this.min(leftIndex(index), rightIndex(index));
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            if (this.contents[i].item().equals(item)) {
                this.contents[i].setPriority(priority);
                this.swim(i);
